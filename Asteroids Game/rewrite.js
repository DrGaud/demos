...
class ShipObject extends GameObject{
    constructor(x,y,r,a,rot,thrusting,thrust){
        ...
        */
        ...
        /**@this this.laserStatus - Flag - Registers if the laser is activly shooting ON or OFF  */
    ...
    shootLasers(){
        ...
        if(this.laserArray.length >= LASER_MAX){
            this.laserArray.push({
                ...
                x:this.x + 4 / 3 * this.r * Math.cos(this.a),
                y:this.y - 4 / 3 * this.r * Math.sin(this.a),
                vX: LASER_SPEED * Math.cos(this.a) / FPS,
                vY: -LASER_SPEED * Math.sin(this.a) / FPS,
                distanceTravelled:0
            })
            //Resetting the array 
            this.laserArray = []
        ...
        for(let astroIndex = reference.asteroidField.length-1; astroIndex >= 0 ; astroIndex--){
            //Grabbing the asteroids properties
            let asteroidX = reference.asteroidField[astroIndex].x 
            let asteroidY = reference.asteroidField[astroIndex].y
            let asteroidR = reference.asteroidField[astroIndex].r
            ...
            for(let laserIndex = this.laserArray.length-1; laserIndex >=0; laserIndex--){
                ...
                if(distanceBetweenPoints(asteroidX,asteroidY,laserX,laserY) < asteroidR){
                    ...
                    //destroy asteroid
                    reference.destroyAsteroid(astroIndex)
...
function gameAnimation(){
    ...
    if(reference.asteroidField.length === 0) reference.createAsteroidField(randomNumber(5,15))
