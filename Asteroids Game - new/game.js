...
let this.currentScore = 0;// Keeps track of the score. 
let lives = 5;
...
function drawShip(obj){
    ...
    for(let astroIndex = reference.asteroidsField.length-1; astroIndex >= 0 ; astroIndex--){
        //Grabbing the asteroids properties
        let asteroidX = reference.asteroidsField[astroIndex].x 
        let asteroidY = reference.asteroidsField[astroIndex].y
        let asteroidR = reference.asteroidsField[astroIndex].r
...
let reference.asteroidsField = [];
let asteroidsNum = 5; //Asteroids Number

function createAsteroidField (){ 
    //Clear the Array
    reference.asteroidsField = [];
    let x,y,r;
    //Loop over the array
    for(let index = 0; index < asteroidsNum; ++index){
        ...
        reference.asteroidsField.push(newAsteroid(x,y,r));
    }
    return reference.asteroidsField;
}
...
function drawAsteroids (){
    ...
    reference.asteroidsField.forEach((asteroid,index) =>{
        //Draw Asteroids
        ...
        function asteroid2asteroidCollision(asteroid,index){
            ...
            for(let index = 0; index < reference.asteroidsField.length; index++){
                if(index != currentAsteroidIndex){
                   //Turns out its easier to just calculate the distance between circles than between polygons. 
                    if(distanceBetweenCircles(currentAsteroid.x,currentAsteroid.y,currentAsteroid.r,reference.asteroidsField[index].x,reference.asteroidsField[index].y,reference.asteroidsField[index].r) <= (reference.asteroidsField[index].r ) ){
                        ...
                        reference.asteroidsField[index].vX = -(reference.asteroidsField[index].vX + (Math.sin(reference.asteroidsField[index].a))  )
                        reference.asteroidsField[index].vY = -(reference.asteroidsField[index].vY + (Math.cos(reference.asteroidsField[index].a)) )
...
function destroyAsteroid(index){
    ...
    let x = reference.asteroidsField[index].x;
    let y = reference.asteroidsField[index].y;
    let r = reference.asteroidsField[index].r;
   ...
   if( r <= 60 && r >=30){
       ...
       reference.asteroidsField.splice(index,1)
       //Create the smaller asteroid debris 
       reference.asteroidsField.push(newAsteroid(x,y,r/2))
       reference.asteroidsField.push(newAsteroid(x,y,r/2))
    }
    if( r <= 30 && r >=15){
